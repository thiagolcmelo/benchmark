...
def qho_numerical(L=100.0, N=2048, dt=1e-19,     wave_length=8.1e-6, nmax=6, precision=1e-2,     iterations=None, max_time=None, eigenstates_au=None):
    """
    ...
    analytical = qho_analytical(L=L, N=N,         wave_length=wave_length, nmax=nmax)
    ...
    if not eigenstates_au:
        ...
        eigenstates_au = np.array([g*sp.legendre(i)(short_grid)             for i in range(nmax)],dtype=np.complex_)
    ...
    for s in range(nmax):
        while True:
            ...
            # gram-shimdt
            for m in range(s):
                proj = simps(eigenstates_au[s] *                     np.conjugate(eigenstates_au[m]), z_au)
            ...
            if (iterations and counters[s] >= iterations)                 or (max_time and timers[s] >= max_time)                 or counters[s] % 1000 == 0:
                # second derivative
                ...
                if (iterations and counters[s] >= iterations)                     or (max_time and timers[s] >= max_time)                     or (not iterations and not max_time                         and precisions[s] < precision):
                    XA = [eigenstates_au[s]]
                    XB = [eigenstates_au_ana[s]]
                    vectors_chebyshev[s] =                         cdist(XA, XB, 'chebyshev')[0][0]
                    vectors_seuclidean[s] =                         cdist(XA, XB, 'seuclidean')[0][0]
                    vectors_sqeuclidean[s] =                         cdist(XA, XB, 'sqeuclidean')[0][0]
                    break
    
    # generate eigenstates for SI
    eigenstates_si = np.array([np.ones(N, dtype=np.complex_)         for i in range(nmax)],dtype=np.complex_)
...
def qho_analytical(L=100.0, N=2048, wave_length=8.1e-6, nmax=6):
    ...
    for n in range(nmax):
        an_au = np.sqrt(1.0/(2.0**n * factorial(n))) *             (mwoh_au/np.pi)**(1.0/4.0)
        psin_au = an_au*np.exp(-mwoh_au*z_au**2/2.0) *             hermite(n)(np.sqrt(mwoh_au)*z_au)
        eigenstates_au.append(psin_au)

        an_si = np.sqrt(1.0/(2.0**n * factorial(n))) *             (mwoh_si/np.pi)**(1.0/4.0)
        psin_si = an_si*np.exp(-mwoh_si*z_si**2/2.0) *             hermite(n)(np.sqrt(mwoh_si)*z_si)
...
# res_numeric_iter = []
# for it in [i * 1e5 for i in range(1,13)]:
#     res_numeric_iter.append(qho_numerical(nmax=n, iterations=it))
# np.savez('res_numeric_iter', res_numeric_iter)
...
res_numeric_time = []
for t in [60 * i for i in range(1,13)]:
    print("Iniciando: %s..." % t)
    res_numeric_time.append(qho_numerical(nmax=n, max_time=t))
    np.savez('res_numeric_time_3_%d' % t, res_numeric_time)
np.savez('res_numeric_time_3', res_numeric_time)
...
# np.savez('res_numeric_prec', res_numeric_prec)
...
#     plt.semilogy([i * 1e5 for i in range(1,5)], chebyshev, label='state_%d' % j)
# plt.legend()
# plt.show()
